...
def data_split(img_path):
    ...
    test_part = random.sample(files,int(len(files)*0.2))
    val_part = random.sample(test_part,int(int(len(files)*0.2)*0.5))
    ...
    for file in files:
        if file in val_part:
            with open("./data/ImageSets/Main/val.txt","a") as val_f:
                val_f.write(file[:-4] + "\n" )
            val_index += 1
        elif file in test_part:
            with open("./data/ImageSets/Main/test.txt","a") as test_f:
                test_f.write(file[:-4] + "\n")
            test_index += 1
        else:
            with open("./data/ImageSets/Main/train.txt","a") as train_f:
